...
module vga_adapter(
	...
	wire [((MONOCHROME == "TRUE") ? (0) : (BITS_PER_COLOUR_CHANNEL*3-1)):0] to_ctrl_colourA;
	wire [((MONOCHROME == "TRUE") ? (0) : (BITS_PER_COLOUR_CHANNEL*3-1)):0] to_ctrl_colourB;
	/* Pixel colour read by the VGA controller */
	//assign to_ctrl_colour = (select_stream == 1) ? to_ctrl_colourA : to_ctrl_colourB;
	select_colourStream SCS(select_stream, to_ctrl_colourA, to_ctrl_colourB, to_ctrl_colour);
	...
	parameter asdfColour = 3'b111;

	asdf	asdf_insta (
	.address ( controller_to_video_memory_addr ),
	.data ( asdfColour ),
	.inclock ( clock ),
	.outclock ( clock_25 ),
	.wren ( gnd ),	 // just reading here, not writing 
	.q ( to_ctrl_colourB )
	);
...
module select_colourStream(select, A, B, outs);
	input select;
	input [2:0] A, B;
	output reg [2:0] outs;
	always@(*) begin 
